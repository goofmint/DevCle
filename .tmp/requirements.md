...
ドメインは `devcle.com` とする。開発時は `devcle.test` を利用。
本システムは、開発者向け施策（DevRel活動）を定量的に可視化・分析する  
**DevRel管理・ROI分析・ファネル分析プラットフォーム** である。

OSS版は自社ホスティングを想定し、SaaS版ではテナント単位の運用とAIサポート機能を提供する。
...
| 機能 | 内容 |
|------|------|
| **匿名データ収集** | PostHog / Google Analytics など外部プラグインからイベント取得 |
| **ステータス表示** | Awareness → Engagement → Adoption → Advocacy |
| **ドロップ率分析** | ファネル段階ごとの離脱率を自動計算 |
| **改善状況トラッキング** | 時系列比較でファネル改善率を算出 |
| **外部イベント連携** | APIルーティングまたはcronでデータ更新 |
