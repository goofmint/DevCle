...
/**
 * Props interface for plugin page components
 */
...
/**
 * Dynamically load component from plugin
 */
function loadPluginComponent(pluginId: string, componentName: string | undefined) {
  if (!componentName) {
    return null;
  }

  return lazy<React.ComponentType<PluginPageProps>>(async () => {
    const module = await import(/* @vite-ignore */ `${pluginId}`);
...
export default function PluginDynamicPage() {
  ...
  const PluginComponent = loadPluginComponent(pluginId, componentName);
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-center text-gray-600 dark:text-gray-400 mb-6">
            The plugin component for this page could not be loaded.
          </p>
        </div>
      </div>
    );
