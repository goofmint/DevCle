 ...
 * Seed API Tokens
 *
 * Creates sample API tokens for testing webhook authentication.
 * Tokens are stored as SHA256 hashes in the database.
 *
 * Token format: drowltok_<32 random characters>
 *
 * Sample tokens:
 * - Active token (webhook:write scope)
 * - Active token with expiration
 * - Expired token
 * - Revoked token
 */
async function seedApiTokens(): Promise<void> {
  const db = getDb();
  ...
  if (!admin) {
    console.error('    ❌ Admin user not found, skipping API tokens');
    return;
  }

  // Sample tokens (plain text - for documentation/testing only)
  // In production, these would be generated by generateToken()
  const sampleTokens = {
    active: 'drowltok_SAMPLE1234567890ABCDEFGHIJ',
    activeWithExpiry: 'drowltok_SAMPLE2234567890ABCDEFGHIJ',
    expired: 'drowltok_SAMPLE3234567890ABCDEFGHIJ',
    revoked: 'drowltok_SAMPLE4234567890ABCDEFGHIJ',
  };

  // Helper function to hash tokens (matches token.service.ts)
  ...
  await db
    ...
    .values([
      // 1. Active token (webhook:write scope)
      {
        tokenId: generateUUID(),
        tenantId: 'default',
        name: 'Production Webhook Token',
        tokenPrefix: sampleTokens.active.substring(0, 16),
        tokenHash: hashToken(sampleTokens.active),
      ...
      // 2. Active token with expiration (future)
      {
        tokenId: generateUUID(),
        tenantId: 'default',
        name: 'Development Token',
        tokenPrefix: sampleTokens.activeWithExpiry.substring(0, 16),
        tokenHash: hashToken(sampleTokens.activeWithExpiry),
        scopes: ['webhook:write', 'api:read'],
      ...
      // 3. Expired token
      {
        ...
        tokenPrefix: sampleTokens.expired.substring(0, 16),
        tokenHash: hashToken(sampleTokens.expired),
        scopes: ['webhook:write'],
        expiresAt: new Date('2024-01-01T00:00:00Z'), // Past date
        createdBy: admin.userId,
        createdAt: new Date('2023-12-01T00:00:00Z'),
        lastUsedAt: new Date('2023-12-15T10:00:00Z'),
        revokedAt: null,
      },
      // 4. Revoked token
      {
        ...
        tenantId: 'default',
        name: 'Revoked Token',
        tokenPrefix: sampleTokens.revoked.substring(0, 16),
        tokenHash: hashToken(sampleTokens.revoked),
        scopes: ['webhook:write'],
        expiresAt: null,
        createdBy: admin.userId,
        createdAt: new Date('2024-06-01T00:00:00Z'),
        lastUsedAt: new Date('2024-06-10T15:00:00Z'),
        revokedAt: new Date('2024-07-01T00:00:00Z'),
      },
    ])
    .onConflictDoNothing();

  console.log('    ✅ API tokens seeded (4)');
  console.log('    ℹ️  Sample tokens (for testing only):');
  console.log(`       Active: ${sampleTokens.active}`);
  console.log(`       With expiry: ${sampleTokens.activeWithExpiry}`);
}
...
async function seed(): Promise<void> {
  ...
  try {
    ...
    const tables = [
      ...
      'tenants', 'api_keys', 'api_tokens', 'system_settings', 'notifications',
      // Core entity tables
    ...
    await sql.unsafe('TRUNCATE TABLE api_tokens CASCADE');
    ...
    await seedApiTokens(); // API tokens for webhook authentication
    ...
    console.log('  - API Tokens: 4 (2 active, 1 expired, 1 revoked)');
